# .pos extracts all of the positive values from a matrix. This code is adapted from the R NMF package
.pos <- function(x){ as.numeric(x>=0) * x }


# .neg extracts all of the negative values from a matrix. This code is adapted from the R NMF package

.neg <- function(x){ - as.numeric(x<0) * x }


#.norm takes the Frobenius norm of a non-negative matrix. This code is adapted from the R NMF package
